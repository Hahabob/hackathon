...
export default function DraggableAisle({
  ...
  const colors =
    colorMap[aisle.color] || colorMap["bg-gray-100 border-gray-500"];
  ...
  const dragHandleStyle: React.CSSProperties = {
    ...
    backgroundImage: `url("data:image/svg+xml,%3csvg width='100%25' height='100%25' xmlns='http://www.w3.org/2000/svg'%3e%3crect width='100%25' height='100%25' fill='none' stroke='${encodeURIComponent(
      colors.border
    )}' stroke-width='2' stroke-dasharray='2%2c2' stroke-dashoffset='0' stroke-linecap='square'/%3e%3c/svg%3e")`,
    cursor: "grab",
    opacity: 0.6,
  };
    ...
    <div ref={setNodeRef} style={containerStyle} {...attributes} {...listeners}>
      ...
      <div style={headerStyle}>
        ...
        >
          ...
          {aisle.products.length > 0 && (
            <span style={productCountStyle}>{aisle.products.length}</span>
          )}
        </div>
      </div>

      {showContents && (
        <div ref={setDroppableRef} style={productListStyle}>
          {aisle.products.length > 0 ? (
            <div>
              {aisle.products.map((product) => (
                <div key={product._id} style={productItemStyle}>
                  <div
                    style={{
                      display: "flex",
                      flexDirection: "column",
            ...
            <div style={emptyStateStyle}>
              {isOver
                ? "Drop product here!"
                : "Drop products here or drag this aisle to the store layout"}
            </div>
          )}
        </div>
      )}
      ...
      {!showContents && (
        ...
        >
          {aisle.products.length > 0 ? (
            <span>
              {aisle.products.length} product
              {aisle.products.length !== 1 ? "s" : ""} assigned
            </span>
          ) : (
